...
import openfl.Lib; // windows thingy
...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['SWAG SWAG!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var plr3:Character = null;
	public var gf:Character = null;
	public var boyfriend:Character = null;

	public var altStagewhat:StageFile;
	...
	public var omissionglow:FlxSprite;
	...
	//funny float array and evil trail mech
	public var FloatArrayIDK:Array<String> = ['expunged', 'b', 'foxy'];
	public var EvilTrailedChars:Array<String> = ['b', 'cheater', 'omiss', 'flying'];
	public var evilTrail:FlxTrail; // sadjakslhdkalsjdhajkhdgfikshdnfbkaNHDFKLJSAHDFLKJASDFdss
	public var evilBf:FlxTrail;

	public var boyfriendCameraOffset:Array<Float> = null;
	public var opponentCameraOffset:Array<Float> = null;
	public var girlfriendCameraOffset:Array<Float> = null;

	var frontBlackScreen:FlxSprite;
	public var doubleactglow:FlxSprite;
	...
	//icon angle
	public var mainAllowChangebetweenIconsOnHealth:Bool = true;
	public var iconP1canAngle:Bool = true;
	public var iconP2canAngle:Bool = true;
	...
	override public function create()
	{
		...
		altStagewhat = StageData.getStageFile(curStage);
		...
		if(SONG.song.toLowerCase() == 'double act') // cool
		{
			plr3 = new Character(-381, 274, 'mini-tails');
		}
		...
		doubleactglow = new FlxSprite(-120, -120).loadGraphic(Paths.image('bgs/GrassPlae/double act thi'));
		doubleactglow.scrollFactor.set();
		doubleactglow.antialiasing = true;
		doubleactglow.screenCenter();
		doubleactglow.visible = false;
		add(doubleactglow);

		omissionglow = new FlxSprite(-120, -120).loadGraphic(Paths.image('bgs/omission/glow'));
		omissionglow.scrollFactor.set();
		omissionglow.antialiasing = true;
		omissionglow.screenCenter();
		omissionglow.visible = false;
		add(omissionglow);
		...
		switch (curStage)
		{
			...
			//custom ass
			case 'grass_plae': new states.stages.Grass();
			case 'grass_plae_night': new states.stages.GrassN();
			case '3dFucked': new states.stages.ThreedFaked();
			case 'b house': new states.unstableexported.BHOUSE();
			case 'cheat': new states.stages.Cheat();
			case 'omiss': new states.stages.Mission(); 
			case 'funnySpin': new states.stages.FloatFunny();
			case 'houseOlderDay': new states.stages.OLDjimmy();
			case 'creationTunnel': new states.stages.PurpleTunnel();
			case 'greenTunnel': new states.stages.GreenTunnel();
		}
		...
		if (stageData.ground_particals){
			#if LUA_ALLOWED
			for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'weeks/swag/PlayState/Partds/'))
				for (file in FileSystem.readDirectory(folder))
				{
					if(file.toLowerCase().endsWith('.lua'))
						new FunkinLua(folder + file);
		...
		#if LUA_ALLOWED
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'weeks/swag/'))
			for (file in FileSystem.readDirectory(folder))
			{
				if(file.toLowerCase().endsWith('.lua'))
					new FunkinLua(folder + file);
		...
		if(EvilTrailedChars.contains(dad.curCharacter.toLowerCase())){
			evilTrail = new FlxTrail(dad, null, 4, 12, 0.3, 0.069);
			insert(members.indexOf(dadGroup) - 1, evilTrail);
			switch (curStage){
				case 'cheat':
					evilTrail.color = FlxColor.GREEN;
				case 'omiss':
					evilTrail.color = FlxColor.BLACK;
			}
		}

		switch (curStage){
			case 'funnySpin':
				evilTrail.color = FlxColor.LIME;
				insert(members.indexOf(dadGroup) - 1, evilTrail);
				evilBf = new FlxTrail(boyfriend, null, 4, 12, 0.3, 0.069);
				evilBf.color = FlxColor.BLUE;
				insert(members.indexOf(boyfriendGroup) - 1, evilBf);
		}
		...
		timeTxt.setFormat(Paths.font("fakecomic.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt = new FlxText(0, healthBar.y + 28, FlxG.width, "", 17);
		scoreTxt.setFormat(Paths.font("fakecomic.ttf"), 17, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		songTxt.setFormat(Paths.font("comic.ttf"), 15, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songTxt.scrollFactor.set();
		songTxt.borderSize = scoreTxt.borderSize;
		songTxt.visible = !ClientPrefs.data.hideHud;
		uiGroup.add(songTxt);
		songTxt.text = curSong + " (" + storyDifficultyText + ") " + " | PE " + MainMenuState.psychEngineVersion;

		botplayTxt = new FlxText(400, timeBar.y + 55, FlxG.width - 800, "OIIIIIIIII YOU IDIOT!!1!!!!!", 32);
		botplayTxt.setFormat(Paths.font("fakecomic.ttf"), 32, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		uiGroup.add(doubleactglow);
		uiGroup.add(omissionglow);

		frontBlackScreen = new FlxSprite(-120, -120).makeGraphic(Std.int(FlxG.width * 100), Std.int(FlxG.height * 150), FlxColor.BLACK);
		frontBlackScreen.scrollFactor.set();
		frontBlackScreen.visible = false;
		uiGroup.add(frontBlackScreen);
	...
	public function reloadTimeBarColors() {
		timeBar.setColors(FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]), FlxColor.BLACK); // WORK WORMK WORKSADJHAJSKJDHAS
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if(FloatArrayIDK.contains(dad.curCharacter.toLowerCase())){
			dad.y += (Math.sin(elapsedtime) * 0.6);	
		}

		switch (SONG.song.toLowerCase()){
		case 'holy-shit-dave-fnfnt': // OH YEA AOWDHDH BOI
			switch (curStep)
			{
				case 1:
		...
		case 'delirium':
			...
			{
				case 0:
					frontBlackScreen.visible = true;
					frontBlackScreen.alpha = 1;
				case 1:
					FlxTween.tween(frontBlackScreen, {alpha:0}, 0.3);
				...
				case 1792:
					FlxTween.tween(frontBlackScreen, {alpha:1}, 0.2);
			}
		case 'inflorescence':
		...
		case 'omission':
			...
			{
				case 0:
					omissionglow.alpha = 0;
					omissionglow.visible = true;
				case 1:
					FlxTween.tween(omissionglow, {alpha:1}, 1.1);
				case 394:
					camZoomSnap = true;
				case 640:
					if (ClientPrefs.data.flashing) FlxG.camera.flash(FlxColor.WHITE, 1);
				...
				case 896:
					...
					camZoomSnap = false;	
				case 1152:
					FlxTween.tween(camHUD, {alpha:1}, 0.9);		
			}
		case 'double act':
			switch(curStep)
			{
				case 0:
					doubleactglow.alpha = 0;
					camZoomSnap = false;
					doubleactglow.visible = true;
				case 2:
					FlxTween.tween(doubleactglow, {alpha:1}, 1.4);
				case 1151:
					if (ClientPrefs.data.flashing) FlxG.camera.flash(FlxColor.WHITE, 1);
					camZoomSnap = true;
				case 1408:
		...
		case 'psychosis':
			switch(curStep){
				case 0:
					camHUD.alpha = 0;
				case 234:
					FlxTween.tween(camHUD, {alpha:1}, 1.1);
				case 1300:
		...
		if(SONG.song.toLowerCase() == 'inflorescence')
		{
			var strumLineY:Float = ClientPrefs.data.downScroll ? (FlxG.height - 150) : 50;
			...
			for(str in playerStrums)
			{
				...
				str.y = strumLineY+(40*Math.sin((elapsedtime*2)+str.ID*2));
			}
		...
		switch (curStage)
		{
			case 'grass_plae_night' | 'b house' | 'creationTunnel': // wont be used for 3dFaked cuz no
            {
            	dad.color = 0xFF878787;
				if (plr3 != null){
					plr3.color = dad.color;
				}
            	gf.color = 0xFF878787;
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		if (mainAllowChangebetweenIconsOnHealth){
			if (healthBar.percent > 80){
				iconP2canAngle = false;
			} else {
				iconP2canAngle = true;
			}
			if (healthBar.percent < 20){
				iconP1canAngle = false;
			} else {
				iconP1canAngle = true;
			}
		}
	...
	public function moveCamera(isDad:Bool)
	{
		...
		else
		{
			...
			defaultCamZoom = altStagewhat.defaultZoom;
	...
	public function zoomCameraOutDaddy(){ // gay
		switch (dad.curCharacter){
			case 'expunged':
				defaultCamZoom = (altStagewhat.defaultZoom - 0.25);
			case 'cheater':
				defaultCamZoom = (altStagewhat.defaultZoom - 0.31);
		}
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		switch (curSong.toLowerCase()){
			case 'dereliction':
				if(ClientPrefs.data.flashing) camHUD.shake(0.0096, 0.1);
				if(ClientPrefs.data.flashing) FlxG.camera.shake(0.0045, 0.1);
			case 'psychosis':
				if(ClientPrefs.data.flashing) camHUD.shake(0.0056, 0.1);
				if(SONG.notes[Math.floor(curStep / 16)].mustHitSection == false && !note.isSustainNote)
				{
					if (!dad.stunned)
					{
						...
						{
							case 0:
								Lib.application.window.move(Lib.application.window.x - 4,Lib.application.window.y + 0);
							case 1:
								Lib.application.window.move(Lib.application.window.x - 0,Lib.application.window.y + 4);
							case 2:
								Lib.application.window.move(Lib.application.window.x - 0,Lib.application.window.y - 4);
							case 3:
								Lib.application.window.move(Lib.application.window.x + 4,Lib.application.window.y + 0);
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(note.noteType == 'Alt & Opponent Strum' && char == plr3) {
				char = dad;
			} 
			...
			if(char != null)
			{
				char.playAnim(animToPlay, true);
				char.holdTimer = 0;
				if (note.noteType == 'Alt & Opponent Strum'){
	...
	override function beatHit()
	{
		...
		if (iconP1canAngle){
			FlxTween.angle(iconP1, -23, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
		}
		if (iconP2canAngle){
			FlxTween.angle(iconP2, 23, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
		}
	...
	public function characterBopper(beat:Int):Void
	{
		...
		if (dad != null && beat % dad.danceEveryNumBeats == 0 && !dad.getAnimationName().startsWith('sing') && !dad.stunned)
			dad.dance();
		if (plr3 != null && beat % plr3.danceEveryNumBeats == 0 && !plr3.getAnimationName().startsWith('sing') && !plr3.stunned)
			plr3.dance();
	}
